...
_flag = struct(
    ...
    pip_whl_linux_libc = lambda x: (str(Label("//python/config_settings:pip_whl_linux_libc")), str(x)),
...
def _test_cp_version_is_selected_when_python_version_is_specified(name):
    _analysis_test(
        name = name,
        dist = {
            "is_cp3.10_none_any": "cp310",
            "is_cp3.8_none_any": "cp38",
            "is_cp3.9_none_any": "cp39",
...
def _test_glibc_versioned(name):
    _analysis_test(
        ...
        want = "glibc",
        config_settings = [
            _flag.pip_whl_linux_libc("glibc"),
