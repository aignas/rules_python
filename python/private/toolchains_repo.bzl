...
def python_toolchain_build_file_content(
    ...
    Args:
        ...
        platforms: the platform definitions.
...
def _toolchain_aliases_impl(rctx):
    ...
    build_contents = "\n\n".join([
        ...
        # TODO @aignas 2024-09-06: PLATFORMS is used inside this macro expansion here
        "gen_python_config_settings()",
        """exports_files(["defs.bzl"])""",
        render.alias(
            name = "python3",
            actual = render.select({
                ":" + p: "@{py_repository}_{platform}//:{name}".format(
                    platform = p,
                    py_repository = rctx.attr.user_repository_name,
                    name = "bin/python3" if "windows" not in p else "python3.exe",
                )
...
interpreter = "@{py_repository}_host//:{python3_binary_path}"
...
""")
    ...
    # TODO @aignas 2024-09-06: PLATFORMS should be not used here
    host_platform = _get_host_platform(os_name, arch, PLATFORMS)
...
def _get_host_platform(os_name, arch, platforms):
    """Gets the host platform.

    Args:
        os_name: the host OS name.
        arch: the host arch.
        platforms: the platforms to choose from.
